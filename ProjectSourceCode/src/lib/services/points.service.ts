...
export class PointsService {
  ...
  async awardPointsForTaskCompletion(taskId: string, userId: string): Promise<PointsHistory> {
  ...
  async awardManualPoints(input: ManualAwardInput, awardedBy: string): Promise<PointsHistory> {
  ...
  async getPointsHistory(userId: string): Promise<Array<PointsHistory>> {
    return getPointsHistoryByUser(this.client, userId);
  }

  async getLeaderboard(limit: number = 100): Promise<Array<LeaderboardEntry>> {

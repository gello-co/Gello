...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const team = await getTeamService(req).updateTeam({
        ...req.body,
        id,
      });
      res.json(team);
...
router.delete("/:id", requireAdmin, async (req, res, next) => {
  try {
    ...
    await getTeamService(req).deleteTeam(id);
    res.status(204).send();
  } catch (error) {
    next(error);
  }
...
router.post("/:id/members", requireManager, async (req, res, next) => {
  try {
    ...
    const user = await getTeamService(req).addMemberToTeam(user_id, id);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await getTeamService(req).removeMemberFromTeam(userId);
      res.json(user);
    } catch (error) {
      next(error);
    }

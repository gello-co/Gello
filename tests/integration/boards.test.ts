...
describe('Boards API', () => {
  ...
  describe('GET /api/boards', () => {
    it('should return boards for the authenticated user', async () => {
      const response = await request(app).get('/api/boards').set('Cookie', memberCookies);
    ...
    it('should return empty array if user has no boards', async () => {
      ...
      const response = await request(app).get('/api/boards').set('Cookie', cookieHeader);
  ...
  describe('PUT /api/boards/:id', () => {
    ...
    it('should update board as manager', async () => {
      const { token: csrfToken } = await getCsrfToken(managerCookies);
      let req = request(app).put(`/api/boards/${boardId}`).set('Cookie', managerCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
      const response = await req.send({
        name: 'Updated Board',
    ...
    it('should reject update by member', async () => {
      const { token: csrfToken } = await getCsrfToken(memberCookies);
      let req = request(app).put(`/api/boards/${boardId}`).set('Cookie', memberCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
  ...
  describe('DELETE /api/boards/:id', () => {
    ...
    it('should delete board as manager', async () => {
      const { token: csrfToken } = await getCsrfToken(managerCookies);
      let req = request(app).delete(`/api/boards/${boardId}`).set('Cookie', managerCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
    ...
    it('should reject delete by member', async () => {
      const { token: csrfToken } = await getCsrfToken(memberCookies);
      let req = request(app).delete(`/api/boards/${boardId}`).set('Cookie', memberCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
      const response = await req;

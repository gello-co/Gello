...
describe('Auth API', () => {
  ...
  describe('GET /api/auth/session', () => {
    ...
    beforeEach(async () => {
      ...
      await createTestUser(sessionEmail, 'password123', 'member', 'Session User');
    });
    ...
    it('should return user session with valid session cookies', async () => {
      ...
      const response = await request(app).get('/api/auth/session').set('Cookie', cookieHeader);
  ...
  describe('POST /api/auth/logout', () => {
    ...
    it('should accept logout request with valid session cookies', async () => {
      ...
      let logoutReq = request(app).post('/api/auth/logout').set('Cookie', cookieHeader);
  ...
  describe('Complete Auth Flow', () => {
    it('should complete full flow: register → login → session → logout', async () => {
      ...
      let cookieStrings: Array<string> = [];
      if (registerCookies && Array.isArray(registerCookies) && registerCookies.length > 0) {

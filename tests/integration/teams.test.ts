...
describe('Teams API', () => {
  ...
  beforeAll(async () => {
    ...
    const _adminUser = await createTestUser(adminEmail, 'password123', 'admin', 'Admin User');
    ...
    const _memberUser = await createTestUser(memberEmail, 'password123', 'member', 'Member User');

    // Login via app endpoint and get cookies
    const { cookieHeader: adminCookieHeader } = await loginAsUser(adminEmail, 'password123');
  ...
  describe('GET /api/teams', () => {
    it('should return all teams for authenticated user', async () => {
      const response = await request(app).get('/api/teams').set('Cookie', memberCookies);
  ...
  describe('GET /api/teams/:id', () => {
    ...
    it('should return team by id', async () => {
      ...
      const response = await request(app).get(`/api/teams/${teamId}`).set('Cookie', managerCookies);
  ...
  describe('POST /api/teams', () => {
    ...
    it('should require authentication', async () => {
      const response = await request(app).post('/api/teams').send({ name: 'Unauthorized Team' });
  ...
  describe('PUT /api/teams/:id', () => {
    ...
    it('should update team as manager', async () => {
      const { token: csrfToken } = await getCsrfToken(managerCookies);
      let req = request(app).put(`/api/teams/${teamId}`).set('Cookie', managerCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
    ...
    it('should reject update by member', async () => {
      const { token: csrfToken } = await getCsrfToken(memberCookies);
      let req = request(app).put(`/api/teams/${teamId}`).set('Cookie', memberCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
  ...
  describe('DELETE /api/teams/:id', () => {
    ...
    it('should delete team as admin', async () => {
      const { token: csrfToken } = await getCsrfToken(adminCookies);
      let req = request(app).delete(`/api/teams/${teamId}`).set('Cookie', adminCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
    ...
    it('should reject delete by manager', async () => {
      const { token: csrfToken } = await getCsrfToken(managerCookies);
      let req = request(app).delete(`/api/teams/${teamId}`).set('Cookie', managerCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
      const response = await req;
    ...
    it('should reject delete by member', async () => {
      const { token: csrfToken } = await getCsrfToken(memberCookies);
      let req = request(app).delete(`/api/teams/${teamId}`).set('Cookie', memberCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
      const response = await req;
  ...
  describe('POST /api/teams/:id/members', () => {
    ...
    beforeEach(async () => {
      ...
      const user = await createTestUser(newMemberEmail, 'password123', 'member', 'New Member');
      userId = user.user.id;
    });

    it('should add member to team as manager', async () => {
      const { token: csrfToken } = await getCsrfToken(managerCookies);
      let req = request(app).post(`/api/teams/${teamId}/members`).set('Cookie', managerCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
    ...
    it('should reject adding member by regular member', async () => {
      const { token: csrfToken } = await getCsrfToken(memberCookies);
      let req = request(app).post(`/api/teams/${teamId}/members`).set('Cookie', memberCookies);
      req = setCsrfHeadersIfEnabled(req, csrfToken);
      const response = await req.send({ user_id: userId });
  ...
  describe('DELETE /api/teams/:teamId/members/:userId', () => {
    ...
    it('should require authentication', async () => {
      const response = await request(app).delete(`/api/teams/${teamId}/members/${userId}`);

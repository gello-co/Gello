...
describe('Points API', () => {
  ...
  beforeAll(async () => {
    ...
    const _admin = await createTestUser(adminEmail, 'password123', 'admin', 'Admin User');
  ...
  describe('GET /api/points/users/:id/points', () => {
    ...
    it('should require authentication', async () => {
      const response = await request(app).get(`/api/points/users/${userId}/points`);
  ...
  describe('POST /api/points/users/:id/points', () => {
    it('should award points manually as admin', async () => {
      ...
      let req = request(app).post(`/api/points/users/${userId}/points`).set('Cookie', adminCookies);
    ...
    it('should require authentication', async () => {
      const response = await request(app).post(`/api/points/users/${userId}/points`).send({
        points_earned: 10,
        reason: 'bonus',
      });
